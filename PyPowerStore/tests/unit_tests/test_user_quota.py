...
class TestUserQuota(TestBase):
    ...
    def test_get_user_quota_with_filter(self):
        ...
        with mock.patch.object(self.provisioning.client,
            ...
            self.provisioning.get_file_user_quotas(filter_dict=querystring,
                                                   all_pages=True)
    ...
    def test_get_user_quota_with_query_param(self):
        ...
        with mock.patch.object(self.provisioning.client,
                               'request') as mock_request:
            self.provisioning.get_user_quota(
                user_quota_id=None, query_params=param)
            mock_request.assert_called_with(
                constants.GET,
                constants.GET_USER_QUOTA_LIST_URL.format(
                    self.provisioning.server_ip),
