...
class SecurityConfigResponse(Entity):
    ...
    def get_security_config_details(self):
        if self.url.endswith('/security_config/{0}'.format(
                self.data.invalid_security_config_id)):
    ...
    def modify_security_config(self):
        if 'protocol_mode' in self.kwargs['data'] and \
                self.kwargs['data']['protocol_mode'] == \
                self.data.invalid_protocol_mode:
